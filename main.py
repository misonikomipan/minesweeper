...
class Grid:
    ...
    def __str__(self):
        # print(obj)での出力
        state = (
            "#" if self.isBomb else self.nAroundBomb) if self.isOpen else "."
        return f"{state}"
...
class MineSweeper:
    ...
    def setBomb(self):
        ...
        bomPosList2d = tuple(
            map(divmod, bombPosList1d,
                [self.nCol for iBomb in range(self.nBomb)]))  # (r, c)
        # 盤面に反映
        # TODO: for文を排除
    ...
    def judgeIsLose(self):
        currentNBombOpen = sum([
            sum([(boardElement.isOpen and boardElement.isBomb)
                 for boardElement in boardRow]) for boardRow in self.board
        ])
    ...
    def judgeIsWin(self):
        ...
        currentNOpen = sum([
            sum([boardElement.isOpen for boardElement in boardRow])
